...
#shard size must be a multiple of the max length
config.shard_size = config.shard_size // (config.max_length) * config.max_length
...
def tokenize(doc):
    ...
    tokens = enc.encode_ordinary(doc[config.text_column])[:config.max_length]
    tokens = tokens + [eot] * (config.max_length - len(tokens))
